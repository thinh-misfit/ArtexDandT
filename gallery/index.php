...
define ( 'SECURITY_PHRASE', 'forever' );
...
function sfpg_clean_data_root($dir) {
	$items = array_diff ( scandir ( DATA_ROOT . 'info/' . $dir ), array (
			'.',
			'..',
			'_sfpg_dir' 
	) );
...
if (PAYPAL_ENABLED and $get_set and ($_GET ['cmd'] == 'buy') and IMAGE != '') {
	...
	if (($sell == false) or ($sell [1] < 1)) {
		...
		fclose ( $fp );
		echo '<body onload="paypal(' . IMAGE_ID_IN_URL . ',\'' . $buyId . '\');">' . TEXT_PAYPAL_PLEASE_WAIT . '<form  name="_xclick" action="https://www.paypal.com/cgi-bin/webscr" method="post">' . '<input type="hidden" name="cmd" value="_xclick">' . '<input type="hidden" name="business" value="' . PAYPAL_ACCOUNT . '">' . '<input type="hidden" name="currency_code" value="' . PAYPAL_CURRENCY . '">' . '<input type="hidden" id="paypalReturn" name="return" value="">' . '<input type="hidden" id="paypalCancelReturn" name="cancel_return" value="">' . '<input type="hidden" id="paypalItemName" name="item_name" value="">' . '<input type="hidden" id="paypalItemNumber" name="item_number" value="' . GALLERY . IMAGE . '">' . '<input type="hidden" id="paypalAmount" name="amount" value="">' . '</form>' . '</body>';
	}
} else {
	echo '<body onresize="initDisplay()" onload="showGallery(' . (IMAGE_ID_IN_URL ? IMAGE_ID_IN_URL : 'false') . ')" class="sfpg">' . '<div id="box_navi" class="box_navi">' . '<table class="sfpg_disp" cellspacing="0">' . '<tr><td class="navi">' . '<div id="navi"></div>' . '</td></tr>' . '<tr><td class="menu">' . '<div id="div_menu"></div>' . '</td></tr>' . '</table>' . '</div>' . 

	'<div id="box_image" class="box_image">' . '<table class="sfpg_disp" cellspacing="0">' . '<tr><td class="mid">' . '<img alt="" src="" id="full" class="full_image" onclick="closeImageView()">' . '</td></tr>' . '</table>' . '</div>' . 

	'<div id="box_wait" class="box_wait">' . '<table class="sfpg_disp" cellspacing="0">' . '<tr><td class="mid">' . '<div id="wait"></div>' . '</td></tr>' . '</table>' . '</div>' . 

	'<div id="box_info" class="box_info">' . '<table class="info" cellspacing="0">' . '<tr><td>' . '<div id="box_inner_info"></div>' . '</td></tr>' . '</table>' . '</div>' . 
